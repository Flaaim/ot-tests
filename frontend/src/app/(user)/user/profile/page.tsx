...
import ChangePersonalDataDialog from "@/components/User/ChangePersonalDataDialog";
...
export default async function ProfilePage() {
  ...
  const getYandexAuthUrl = (isAttach = false) => {
    ...
    const redirectUri = isAttach
      ? process.env.NEXT_PUBLIC_YANDEX_ATTACH_REDIRECT_URI
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
              ...
              <Button variant="secondary" size="sm">
                <Link href="/user/profile/change-password">Изменить пароль</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
              ...
              {isYandexAttached ? (
                ...
                <Button variant="secondary" size="sm">
                  <Link href={getYandexAuthUrl(true)}>Привязать</Link>
                </Button>
              )}
            </div>
          </CardContent>
